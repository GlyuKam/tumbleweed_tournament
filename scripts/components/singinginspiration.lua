...
function SingingInspiration:FindFriendlyTargetsToInspire()
	...
	local all_targets = FindPlayersInRange(x, y, z, self.attach_radius, true) or { self.inst }
...
local function HasFriendlyLeader(target, singer, PVP_enabled)
    ...
    return  (target_leader ~= nil and (target_leader == singer or target_leader:HasTag("player")))
			or target.components.domesticatable and target.components.domesticatable:IsDomesticated()
			or target.components.saltlicker and target.components.saltlicker.salted
end
        ...
        if songdata.CUSTOMTARGETFN then
            ...
            for _, ent in ipairs(entities_near_me) do
                if self.inst.components.combat:CanTarget(ent)
                    and not HasFriendlyLeader(ent, self.inst, false)
